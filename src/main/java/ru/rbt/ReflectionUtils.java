...
public class ReflectionUtils {
     ...
     */
    public static Class getGenericClass(final Class actualClass, final Class genericClass) {
        ...
        if (!genericClass.isAssignableFrom(actualClass.getSuperclass())) {
            throw new IllegalArgumentException("Класс " + genericClass.getName() + " не является предком "
                    + actualClass.getName() + ".");
        }
        ...
        while (true) {
            ...
            if (isParameterizedType) {
                ...
                // В иерархии встретился не параметризованный класс. Все ранее сохраненные параметризованные классы будут бесполезны.
        ...
        try {
            result = genericClasses.pop().getActualTypeArguments()[0];
